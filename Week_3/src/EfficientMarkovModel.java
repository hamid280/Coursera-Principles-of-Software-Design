...
public class EfficientMarkovModel extends AbstractMarkovModel {
    ...
    public String getRandomText(int numChars) {
        ...
        String current = myString.substring(index, index+myOrder);
        sb.append(current);
    ...
    private void buildMap() {
        for (int i = 0; i < myString.length() - (myOrder-1); i++) {

            String current = myString.substring(i, i+myOrder);
//			System.out.println(current);
            String follow ="";
            if(i+myOrder<myString.length())
                follow = myString.substring(i+myOrder, i+myOrder+1);
